...
```{r}
# plotting a single spatial object of the world. 
...
```{r}
# plotting vector a nd raster spatial objects
...
```{r}
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # add bounding box to restrict map extent
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
```
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              ...
              fill.legend = tm_legend(title = "Life Exepectancy (years"))
...
```
```{r}
tmap_mode("plot") # or view to interact
```
...
map1 <- tm_shape(world_vector) +
  tm_polygons(fill = "gdp_per_cap",
              fill.legend = tm_legend(title = "GDP Per Capita"))
tmap_save(map1, here::here("week1, "~/Desktop))
```
...
map1 <- tm_shape(world_vector) +
  tm_polygons(fill = "gdp_per_cap",
              fill.legend = tm_legend(title = "GDP Per Capita"))
tmap_save(map1, here("tmap_1.png"))
